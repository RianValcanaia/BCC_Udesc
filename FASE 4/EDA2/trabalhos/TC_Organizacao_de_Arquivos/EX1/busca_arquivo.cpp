...
void carrega_arquivo(vector<Spessoa> &agenda, string nome_arquivo, bool *continuar){
    ifstream arq(nome_arquivo);

    if (!arq) {
        cout << "\nArquivo de agenda não encontrado. Finalizando..." << endl;
        *continuar = false;
        return;
    }
...
int main(){
    ...
    carrega_arquivo(agenda,arq, &continuar);
