...
Programa
    : ListaFuncoes BlocoPrincipal { case $2 of 
...
Funcao  
    : TipoRetorno id '(' DeclParametros ')' BlocoPrincipal    {FunDef ($2 :->: ($4, $1)) $6}
...
-- === DECLARAÇÕES SIMPLES (SEM INICIALIZAÇÃO) ===
...
-- Declaração: Retorna apenas a lista de Variáveis 
Declaracao : Tipo ListaId ';' 
{ map (\nome -> nome :#: ($1, 0)) $2 }

-- Lista de Declarações: Retorna [Var]
Declaracoes 
...
BlocoPrincipal  
    -- Caso 1: Declarações E Comandos
    : '{' Declaracoes ListaCmd '}'  { BlocoPrinc $2 $3 }
    
    -- Caso 2: Só Comandos
    | '{' ListaCmd '}'              { BlocoPrinc [] $2 }
    
    -- Caso 3: Só Declarações (útil para testes simples)
    | '{' Declaracoes '}'           { BlocoPrinc $2 [] }
...
ListaParametros 
    : ListaParametros ',' Expr    {$1 ++ [$3]}
    | Expr                        {[$1]}

{
data BlocoPrinc = BlocoPrinc [Var] Bloco
data FuncaoDefinicao = FunDef Funcao BlocoPrinc

parseError :: [Token] -> a
parseError s = error ("Erro de sintaxe no token: " ++ show (head s))
...
main = do putStr "Informe o nome do arquivo "
          arquivo <- getLine
          s <- readFile arquivo
