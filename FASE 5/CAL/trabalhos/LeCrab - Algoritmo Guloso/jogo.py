...
class Enemy(Element):
    ...
    def nextNode(self):
        ...
        my_node = self.lastNode

        movimentos_possiveis = []

        # CIMA
        cima = (my_node[0], my_node[1] - 1)
        if self.isSafeNode(cima[0], cima[1]):
            dist = abs(cima[0] - player_node[0]) + abs(cima[1] - player_node[1])
            movimentos_possiveis.append((cima, dist))
            
        # BAIXO
        baixo = (my_node[0], my_node[1] + 1)
        if self.isSafeNode(baixo[0], baixo[1]):
            dist = abs(baixo[0] - player_node[0]) + abs(baixo[1] - player_node[1])
            movimentos_possiveis.append((baixo, dist))
            
        # ESQUERDA
        esquerda = (my_node[0] - 1, my_node[1])
        if self.isSafeNode(esquerda[0], esquerda[1]):
            dist = abs(esquerda[0] - player_node[0]) + abs(esquerda[1] - player_node[1])
            movimentos_possiveis.append((esquerda, dist))

        # DIREITA
        direita = (my_node[0] + 1, my_node[1])
        if self.isSafeNode(direita[0], direita[1]):
            dist = abs(direita[0] - player_node[0]) + abs(direita[1] - player_node[1])
            movimentos_possiveis.append((direita, dist))

        # Se não houver movimentos possíveis (preso em um beco)
        if not movimentos_possiveis:
            return None
            
        # Ordena a lista de movimentos com base na distância 
        # O movimento com a menor distância será o primeiro da lista
        movimentos_possiveis.sort(key=lambda x: x[1])

        # A escolha gulosa é sempre o nó com a menor distância
        melhor_movimento = movimentos_possiveis[0][0]
